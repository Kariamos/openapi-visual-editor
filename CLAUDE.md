...
- **`doubleQuotedMinMultiLineLength: MAX_SAFE_INTEGER` on every emit** (`EMIT_OPTS` in `yamlSurgicalPatch.ts`, and the fallback `toString()`): `lineWidth: 0` only disables width-based wrapping — it does NOT stop eemeli from folding a double-quoted scalar that contains newlines into a multi-line block (threshold defaults to 40 chars). Stoplight descriptions written as `"…\r\n…"` would otherwise come back as folded blocks with trailing `\r` and blank continuation lines: same value, huge diff.
